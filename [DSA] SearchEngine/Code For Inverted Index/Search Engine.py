...
#This function will connect with the mysql database and upload the indexes the relations are defined on the database 
def store(docID,path,title,frequency):
    
    conn = mysql.connect(host = 'localhost', user = 'root', password = '', db='indexes');
    execute_query = conn.cursor();
    sqlquery = """INSERT IGNORE INTO documents (docID,path,title) VALUES (%s,%s,%s)"""
    execute_query.execute(sqlquery,(docID,path,title));
    unidict = [k.decode('utf8') for k, v in frequency.items()]
    str1 = ' '.join(unidict)

    query2 = """INSERT INTO contents (docID,content) VALUES (%s,%s)""";
    execute_query.execute(query2,(docID,str1));
    conn.commit();
#     values = ','.join(map(str,frequency.items()));
#       print(values+(str(id),));
...
#     print(values);
    #execute_query.execute("""INSERT INTO words(word,frequency,docID) VALUES{}""".format(values));
    #Using execute many commands to insert whole list at once
    execute_query.executemany("""INSERT INTO words(word,frequency,docID) VALUES(%s,%s,%s)""",freq_list);
#     print(values);
...
#     print(a);
        #sqlquery2 = "INSERT INTO words(word,frequency,docID) VALUES ('" + key + "',frequency ,id)"
#         sqlquery2 = """INSERT INTO words(word,frequency,docID) VALUES(%s,%s,%s)"""
        
        
#     sqlquery2 = "";
#     query.execute("INSERT INTO words VALUES"+string);
    #conn.commit();
...
#         print(key,value);
    return frequency;
#This function will clean the list obtained from beautifulsoup object by removing unwanted words
def cleanlist(wordlist):
    newlist = [];
    accepted = "abcdefghijklmnopqrstuvwxyz\'"  
    #These are the stopping words
    ...
    for word in wordlist:
        ...
        if(len(word) == 1):
            ...
            if(word in words_to_be_ignored):
                ...
                newlist.append(word); #Here we are encoding unicode string to ascii
              
                
    return newlist;#Returning new list containing useful words
    
docID = 0;
for root,dirs,files in os.walk(r'C:\wikipedia-simple-html\wikipedia-simple-html\simple\articles'):
    os.chdir(root);
    for file in files:
        if file.endswith('.html'):
            path = os.path.join(root,file);
            #Filtering unwanted HTML files
            ...
            if(soup.find('div',{'id': 'bodyContent'}) == None):
                continue;
            else:
                docID+=1;
                title = soup.h1.text;
                bodycontent = soup.find('div',{'id': 'bodyContent'}).text.encode('ascii','ignore');
                wordlist = bodycontent.lower().split();
                cleanedlist = cleanlist(wordlist);
                freq = countfrequency(cleanedlist);
                store(docID,path,title,freq);
                
               # print(bodycontent.lower().split());
#                 cont = [];
#                 for content in bodycontent.findAll('p'):
#                    for space in content.text:
#                     if space == '':
#                         continue;
#                    cont.append(content.text.encode('ascii','ignore').lower());
                
#                 wordstream = ''.join(map(str,cont));
#                 print(re.sub(' +', ' ',wordstream));
                #print(cont);
#                     print(content.text.encode('ascii','ignore'));
#                     cont.append(content.text.encode('ascii','ignore').lower());
#                 wordstream =' '.join(x.strip() for x in cont if x.strip());
#                 print(wordstream);
#                 wordlist = wordstream.split();
#                 cleanedlist = cleanlist(wordlist);
#                 freq = countfrequency(cleanedlist);
#                 store(docID,path,title,freq);
